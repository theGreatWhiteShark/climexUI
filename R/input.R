...
##'     appear  in `data.frame.positions$name`  will be  discarded and
##'     vice versa.
##'
...
##'     input has the right format) and a version of the input
##'     stripped of any list elements and data.frame columns, which
...
check.input <- function( list.time.series, data.frame.positions ){
    ...
    if ( !is.data.frame( data.frame.position ) ){
        warning( "data.frame.position has to be a data.frame!" )
        check.result <- FALSE
    }
